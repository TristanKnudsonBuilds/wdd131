...
// Returns a random whole number between 0 and the number provided.
function random(number) {
  return Math.floor(Math.random() * number);
}

// Creates the HTML for all tags belonging to a recipe.
...
// Creates an accessible five-star rating.
...
// Displays every recipe contained in the provided array.
function renderRecipes(recipeList) {
  ...
  if (recipeList.length === 0) {
    recipeContainer.innerHTML =
      '<p class="no-results">No matching recipes were found.</p>';
    return;
  }
...
// Finds recipes whose name, description, or tags match the search.
function filterRecipes(query) {
  ...
  const filteredRecipes = recipes.filter(function (recipe) {
    ...
    const tagMatches = recipe.tags.some(function (tag) {
  ...
  filteredRecipes.sort(function (firstRecipe, secondRecipe) {
...
// Displays one random recipe when the page first loads.
function init() {
  const randomRecipe = recipes[random(recipes.length)];
...
searchForm.addEventListener("submit", function (event) {
  ...
  const searchResults = filterRecipes(searchInput.value);
